...
function processLogisticsMessageV2_(message) {
  ...
  attachments.forEach(function (attachment) {
    ...
    blob.setName(name);
    // Archive the actual shipping documents, not the zip container: unzip
    // here and queue the extracted files for Drive upload individually, so
    // e.g. a "STY-2226-docs.zip" attachment results in the BOL/invoice/etc.
    // files landing in the shipment's Drive folder as their own files.
    if (/\.zip$/i.test(name.toLowerCase()) || /zip/i.test(blob.getContentType())) {
      try {
        Utilities.unzip(blob).forEach(function (inner) { documentAttachments.push(inner); });
      } catch (zipErr) {
        writeLog_("GMAIL V2 ARCHIVE", meta.messageId, "ZIP could not be opened for archiving: " + zipErr.message);
  ...
  if (documentAttachments.length) {
    documentFolderUrl = archiveEmailAttachmentsV2_(documentAttachments, records, context, meta);
...
function archiveEmailAttachmentsV2_(attachments, records, context, meta) {
  try {
    var kind = context.kind === "outbound" ? "outbound" : "inbound";
    var folder = (kind === "outbound" ? findExistingOutboundDocsFolderV2_(records) : findExistingInboundDocsFolderV2_(records))
      || getOrCreateShipmentDocsFolderV2_(records, context, meta);
...
function findExistingOutboundDocsFolderV2_(records) {
  if (!records || !records.length) return null;
  var sheet = SpreadsheetApp.openById(GMAIL_PIPELINE.masterId).getSheetByName("WH Trucking Request");
  ...
  for (var r = 2; r < values.length; r++) {
    ...
    records.forEach(function (record) {
      var score = 0;
      if (sameEmailIdV2_(values[r][18], record.pro)) score += 120;
      if (multilineHasV2_(values[r][1], record.invoice)) score += 80;
  ...
  var rich = sheet.getRange(hits[0].row, 1).getRichTextValue();
...
function getOrCreateShipmentDocsFolderV2_(records, context, meta) {
  ...
  var base = names.length === 1 ? names[0]
...
function upsertOutboundEmailV2_(record, allowInsert) {
  ...
  if (candidates.length && (!candidates[1] || candidates[0].score > candidates[1].score)) {
    ...
    if (record._driveFolder && setOutboundDocsLinkV2_(sheet, rowNumber, old[0] || record.customer, record._driveFolder)) changed = true;
  ...
  setOutboundDocsLinkV2_(sheet, insertedRow, record.customer, record._driveFolder);
  return { matched: true, action: "inserted", row: insertedRow };
}
...
function emailNoteV2_(record) {
  var subject = String(record._emailSubject || "").trim();
  var link = String(record._sourceEmail || "").trim();
  var parts = [];
  if (subject) parts.push("Auto-extracted from " + subject);
  if (link) parts.push("[auto: " + link + "]");
  return parts.join(" ");
}
