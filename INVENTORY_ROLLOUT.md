...
- `bobo61480/logistics` on `main` is the authoritative source for the public frontend.
- Every push to `main` is built and published by `.github/workflows/deploy-planner.yml`.
- `public/CNAME` must remain `stylekorean.dpdns.org`; the custom domain is served by GitHub Pages.
- Status changes use the Sites `/api/status` proxy so the browser receives readable JSON confirmation before the UI changes. The client still re-resolves the physical Sheet row and rereads the persisted status after every write.
- The Sites deployment is a backend proxy and tested fallback, not a competing owner of the custom domain.

Production updates are complete only after the GitHub Pages workflow succeeds and the custom domain is checked for the expected frontend assets and functions.
...
- Shipment references shown beneath inbound product names open the shipment-specific packing list when an exact mapping exists, then fall back to the shipment document folder or a Drive packing-list search.
...
GitHub Pages deploys the frontend only. The Sites status proxy forwards confirmed writes to the separate production Apps Script web app. To activate or update the receive-to-stock handler and the Drive v3 scanner fix, update that Apps Script project with the files in `google-apps-script/`, redeploy its web app, run `setupAllTriggers()` once, and then reprocess or approve the pending rows. Keep the Advanced Drive service enabled as Drive API v3.
