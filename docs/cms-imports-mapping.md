...
resolved in `TB_INVC`, so the CMS can pre-fill or reconcile the IMPORTS columns
that are currently maintained by hand.

**Access-grade note (2026-08-28):** the `TB_PNFM` enrichment below (actual
arrival, received qty, confirmation no) depends on the caller's data-access
grade. In the 2026-08-26 session those columns returned fully; in a later
session the gateway masked `TB_PNFM.invc_no` to `null` for the caller (a
row-scope/masking policy — not something to work around), so the join key was
unavailable and the script fell back to the `TB_INVC`-only columns. The runner
degrades gracefully: when `TB_PNFM` yields nothing for an invoice, the actual
...
reference): `invc_qtot` (shipped qty), `iw_qtot` (received qty — from `TB_PNFM`,
access-gated per the note above; matched shipped exactly on all 30 in the
2026-08-26 session), `invc_icnt` (SKU count), `invc_atot` (amount,
